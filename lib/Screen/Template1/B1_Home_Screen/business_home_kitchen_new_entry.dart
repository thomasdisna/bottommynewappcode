...
import 'Business_HomeKitchen_Add_Product_Page.dart';
...
var pageIdPass;
class HomeKitNewEntryForm extends StatefulWidget {

  HomeKitNewEntryForm({this.pagid,this.timid,this.memberdate});
  String pagid,timid,memberdate;

  @override
  _HomeKitNewEntryFormState createState() => _HomeKitNewEntryFormState();
}

class _HomeKitNewEntryFormState extends StateMVC<HomeKitNewEntryForm> {
  ...
  _HomeKitNewEntryFormState() : super(HomeKitchenRegistration()) {
    _con = controller;
  }
  getHomeKitpageTime() async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    pageIdPass = prefs.getString('HK_page_id').toString();
    pageid_HomeKitchen = prefs.getString('HK_page_id');
    timelineid_Homekitchen = prefs.getString('HK_tim_id');
  }
  ...
  _imagBlogUpload() {
    _location.toString().length > 5 && _utube.text == null && _utube.text.isEmpty
        ...
        _location, lat, lng,widget.timid,widget.memberdate,"1")
        ...
        _location, lat, lng,widget.timid,widget.memberdate,"1",_utube.text) :  _location.toString().isEmpty && _utube.text != null &&
        _utube.text.isNotEmpty ? _con.postBlogPost3(context,widget.pagid.toString(), _desc.text,_blogTitle.text,
        IMAGELISTS.join(",").toString(), "image", "blog",widget.timid,widget.memberdate,"1",_utube.text) : _con.postBlogPost2(context,widget.pagid.toString(),
        _desc.text,_blogTitle.text, IMAGELISTS.join(",").toString(), "image", "blog",widget.timid,widget.memberdate,"1");
    typ = "";
    _video = null;
    // _uploadTask=null;
    return Container();
  ...
  _imagUpload() {
    ...
    _location.toString().length > 5
        ...
        IMAGELISTS.join(",").toString(), "image", _location, lat, lng,widget.timid,widget.memberdate,"1")
        : _con.posttimelineWallPost2(context,
     widget.pagid,
      _desc.text,
      IMAGELISTS.join(",").toString(),
      "image",widget.timid,widget.memberdate,"1"
    );
                          ...
                          if (_desc.text.length > 0 && typ == "") {
                            ...
                            if (_desc.text.contains("https") == true ||
                                      ...
                                      FetchPreview().fetch(url).then((res) {
                                        setState(() {
                                          ...
                                          _location.toString().length > 5
                                              ...
                                              data['image'].toString(),
                                              data['title'].toString(),
                                              data['description'].toString(),data['site'].toString(),url,widget.timid,widget.memberdate,"1")
                                              : _con.posttimelineDescPostp(context,
                                            widget.pagid,
                                              _desc.text,
                                              data['image'].toString(),
                                              data['title'].toString(),
                                              data['description'].toString(),data['site'].toString(),url,"1",widget.timid,widget.memberdate,"1");
                                        });
                                      });
                                    });
                                  }
                              ...
                              _location.toString().length > 5
                                  ...
                                  _desc.text, _location, lat, lng, "", "", "","","",widget.timid,widget.memberdate,"1")
                                  : _con.posttimelineDescPost1(context,
                                  widget.pagid, _desc.text, "", "", "","","",widget.timid,widget.memberdate,"1");
                            }
                            ...
                            if (video_type == "utube") {
                              _location.toString().length > 5
                                  ...
                                  _location,
                                  lat,
                                  lng,widget.timid,widget.memberdate,"1")
                                  : _con.posttimelineUTubeVideoPost2(context,
                                widget.pagid,
                                _desc.text,
                                _utube.text,widget.timid,widget.memberdate,"1"
                              );
                              Fluttertoast.showToast(
                                msg: " Uploading Post ",
                                toastLength: Toast.LENGTH_SHORT,
                          ...
                          GestureDetector(
                            onTap: () {
                              Navigator.push(context, MaterialPageRoute(
                                  builder: (context)=>BusinessHomeKitchenAddItemPage(timid: widget.timid,memberdate: widget.memberdate,pagid: widget.pagid,)
                              ));
                            },
                            child: Column(
                              children: <Widget>[
