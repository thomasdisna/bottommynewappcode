...
import 'package:Butomy/FIREBASE/business_chatroom.dart';
...
class BusinessHomeKitchenChatList extends StatefulWidget {
  BusinessHomeKitchenChatList({this.myID, this.myName,this.timid,this.memberdate,this.pagid});

  String myID;
  String myName,timid,pagid,memberdate;

  @override
  _BusinessHomeKitchenChatListState createState() => _BusinessHomeKitchenChatListState();
}

class _BusinessHomeKitchenChatListState extends StateMVC<BusinessHomeKitchenChatList> {
  TimelineWallController _con;

  _BusinessHomeKitchenChatListState() : super(TimelineWallController()) {
    _con = controller;
  }
...
print("ggggggggggggggggg "+businesschatcount.toString());
    return Scaffold(
      appBar: AppBar(
        ...
        child: Padding(
          ...
          child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                return countChatListUsers(widget.timid.toString(), snapshot) > 0
                    ? Container(alignment: Alignment.topCenter,
                  child: ListView(primary: true,
                      children: snapshot.data.documents.reversed.map((data) {
                        if (data['userId'] ==widget.timid.toString()) {
                          return Container();
                        } else {
                          return StreamBuilder<QuerySnapshot>(
                              stream: Firestore.instance
                                  .collection('users')
                                  .document(widget.timid.toString())
                                  .collection('chatlist')
                                        ...
                                        trailing: Padding(
                                            ...
                                            child: (chatListSnapshot.hasData && chatListSnapshot.data.documents.length > 0)
                                                ...
                                                stream: Firestore.instance
                                                    ...
                                                    .where('idTo',isEqualTo: widget.timid)
                ...
                child: FloatingActionButton(
                  onPressed: () {
                    Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                            builder: (context) => HomeKitNewEntryForm(
                                  timid: widget.timid,
                                  pagid: widget.pagid,
  ...
  Future<void> _moveTochatRoom(selectedUserToken, selectedUserID, selectedName,
    ...
    try {
      String chatID = makeChatId(widget.timid, selectedUserID);
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => BusinessChatRoom(
                  widget.timid,
                  widget.myName,
                  selectedUserToken,
                  selectedUserID,
